...
secp.hashes.sha256 = (msg: Uint8Array) => createHash("sha256").update(msg).digest();
secp.hashes.hmacSha256 = (key: Uint8Array, msg: Uint8Array) => createHmac("sha256", key).update(msg).digest();

// const wordlist = readFileSync("./public/crypto/bip39-wordlist.txt", "utf-8").split(/\n/g);
...
/**
 * Kelas ECCWallet
 * Menyediakan pembuatan mnemonic, kunci privat, kunci publik,
 * tanda tangan digital, dan verifikasi.
 */
export class ECCWallet {
  /**
   * Membuat mnemonic 24 kata (BIP-39).
   * Secara ilmiah: 256-bit entropy → checksum → mapping ke wordlist.
   */
  static generateMnemonic(): string {
    const entropy = randomBytes(32); // 256-bit entropy
  ...
  /**
   * Menghasilkan private key dari mnemonic.
   * Secara ilmiah: mnemonic → seed → hash → scalar ECC.
   */
  static mnemonicToPrivateKey(mnemonic: string): Uint8Array {
    ...
    // Normalisasi hash menjadi secret key ECC valid
    return secp.utils.randomSecretKey
      ? secp.utils.randomSecretKey()
      : secp.utils.isValidSecretKey(hash)
        ? hash
   ...
   * Secara matematis: P = d × G (perkalian titik kurva eliptik).
   */
  ...
  /**
   * Menandatangani data.
   * Secara ilmiah: hash(data) → ECDSA signature.
   */
  static async sign(
    message: string,
   ...
   * Secara ilmiah: membuktikan kepemilikan private key
  ...
  /**
 * Enkripsi data.
 * Secara ilmiah: ECDH → KDF → symmetric encryption.
 */
  static encrypt(
    message: Uint8Array,
    receiverPublicKey: Uint8Array
  ): { ciphertext: Uint8Array; ephemeralPublicKey: Uint8Array } {
    const ephPriv = secp.utils.randomSecretKey();
    const ephPub = secp.getPublicKey(ephPriv, true);

    const shared = secp.getSharedSecret(ephPriv, receiverPublicKey, true);
    const key = createHash("sha256").update(shared).digest();

    const ciphertext = message.map((b, i) => b ^ key[i % key.length]);
    return { ciphertext, ephemeralPublicKey: ephPub };
  }

  /**
   * Dekripsi data.
   * Secara ilmiah: ECDH → KDF → symmetric decryption.
   */
  static decrypt(
    ...
    return ciphertext.map((b, i) => b ^ key[i % key.length]);
  }

  /**
 * Mengubah string menjadi Uint8Array.
 * Secara ilmiah: encoding simbol Unicode → representasi byte (UTF-8).
 */
  ...
  /**
   * Mengubah Uint8Array menjadi string.
   * Secara ilmiah: decoding byte sequence → simbol Unicode (UTF-8).
   */
  ...
  /**
 * Mengubah hex string menjadi Uint8Array.
 * Secara ilmiah: representasi basis-16 → representasi byte (8-bit).
 */
  ...
  /**
   * Mengubah Uint8Array menjadi hex string.
   * Secara ilmiah: representasi byte → representasi basis-16.
   */
...
interface Geolocation {
  status: "granted" | "denied" | "prompt" | "unsupported" | "error";
  error: string;
  accuracy: string;
  latitude: string;
  longitude: string;
...
export async function POST(request: Request) {
  ...
  try {
    const cookiesValue = await cookies();
    if (cookiesValue.get(COOKIE.AUTH)?.value) {
      const auth = await models.Authentication.findOne({
        where: {
          session: cookiesValue.get(COOKIE.AUTH)?.value || "",
        }
      });
      if (auth) {
        return NextResponse.json(cookiesValue.get(COOKIE.AUTH)?.value, {
          status: 201,
    ...
    const geo: Geolocation = await request.json();
    ...
    const auth = await models.Authentication.create({
      ...
      sec_fetch_dest: headers["sec-fetch-dest"],
      sec_fetch_mode: headers["sec-fetch-mode"],
      ...
      x_forwarded_port: headers["x-forwarded-port"],
    ...
    const protocol = (request.headers.get("x-forwarded-proto") || request.url.split(":")[0]) === "https" ? true : false;
    response.cookies.set(COOKIE.AUTH, auth.session, {
      httpOnly: true,
      secure: protocol, // hanya HTTPS akan secure
      sameSite: "lax",
      path: "/",
    });
